...
*/
	...
	if ( $id === false ) {
		...
		if ( $n == 1 ) {
			...
			if ( $cWorker[ 'actual' ] != 1 ) {
				$expFirstID = $cWorker[ 'first_id' ];
				$tmpArr = array_values(
					array_filter( $tabWorkers , function( $el ) use ( $expFirstID ) {
							return ( $el[ 'first_id' ] == $expFirstID && $el[ 'actual' ] == 1 );
					} )
				);
				if ( count( $tmpArr ) == 1 ) {
					$cWorker = $tmpArr[ 0 ];
					$dataExpID = $cWorker[ 'id' ];
				}
			}
			$cTime = time();
		...
		//pay_{date,details}
			$portalDB->insertRow( 'expertize' , array(
				...
				'reason_2' => 0 ,
				'state' => 0 ,
				'order_date' => str_replace( '-' , '\\' , date( 'Y-m-d' , $cTime ) ),
				'price' => $data->d[ 'p' ] / 100.0 ,
				'pay_date' => 'Квитанция к договору '.$docNum ,
				'pay_details' => cvtr( ''.$data->at )
			) );
			...
			Redirect( "/maindb/order-2-dmtx.php?id=".$v_id );
