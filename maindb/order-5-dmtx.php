...
*/
	...
	$res = $portalDB->query( "select `t1`.`name` , `t1`.`dep` , `t2`.`short_name` from `workers` as `t1` , `departments` as `t2` where ( `t1`.`dep` = `t2`.`id` ) and ( `t1`.`actual` <=> 1 )" );
	...
	$doc->setMainContext()->setTextAlign( TEXT_ALIGN_CENTER )->setFontSize( '8pt' )
		...
		->addTextLine( inForm( $dbConfig[ 'org.name.full.type' ] , 1 ).' '.inForm( $dbConfig[ 'org.name.full.name' ] , 1 ).' '.inForm( $dbConfig[ 'org.name.full.head' ] , 2 ) )
	...
	$tbl = $doc->addTable();
		...
		$r->height = '6mm' ;
			...
			$doc->setTableCellContext( $c )->setTextAlign( TEXT_ALIGN_LEFT )->setFontSize( '9pt' )
				->addText( 'Поручаю начальнику '.$depName.' организовать производство экспертизы (исследования) №' );
		...
		$r->height = '10mm' ;
			...
			$doc->setTableCellContext( $c )->setTextAlign( TEXT_ALIGN_LEFT )->setFontSize( '9pt' )
				->addTextLine( 'и взять на контроль поступление денежных средств или исполнение ст.108 АПК РФ, ст.96 ГПК РФ, ст.109 КАС РФ, за исключением экспертиз выполняемых в рамках государственного задания.' )
				->setFontSize( '11pt' )
				->addTextLine( 'Установить срок производства экспертизы (исследования) до: _________________.' )
				->setFontSize( '9pt' )->addTag( 'i' )
				->addText( 'Эксперт предупреждается об уголовной ответственности за дачу заведомо ложного заключения по ст. 307 УК РФ. Эксперту разъясняются его права и обязанности, предусмотренные 73-ФЗ ст. 16,17, ст. 57 УПК РФ, ст. 55 АПК РФ, ст. 85 ГПК РФ, ст. 25.9 КоАП РФ, ст. 49 КАС РФ.' )
				->addTag( 'i0' );
