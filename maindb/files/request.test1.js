 ...
 */
	...
	$.windowOnLoad.push( function() {
		...
		var doc = sendXML( "<docVar />" , false , "" , "tmpl=" + docTemplateID + "&type=" + docType + "&id=" + docID );
		$.docVar = JSON.parse( getXMLNodeValue( doc ) );
		
		$.currentDoc = {
			documentArea : document.getElementById( "document-area" ) ,
			pageArea : document.getElementById( "page-area" ) ,
			dataArea : document.getElementById( "data-area" )
		};
		
		$.currentDoc.pages = [ {
			dom : $.currentDoc.pageArea
		} ];
	...
	function doSave( tid ) {
		...
		var doc = sendXML( "<save id=\"" + tid + "\">" + toCDATA( res ) + "</save>" , false , "" , "tmpl=" + docTemplateID + "&type=" + docType + "&id=" + docID );
	}
	
	function doGetRTF( t , y , e ) {
		var variables = {};
		variables = JSON.stringify( variables );
		var idbg = document.getElementById( "req-dbg-mode" );
		window.location.assign( "request.rtf.php?tmpl=" + t + "&type=" + y + "&id=" + e + ( idbg != null && idbg.checked ? "&dbg=1" : "" ) );
	}
	
	function getPagesCount(  ) {
		var cd = $.currentDoc ;
		var da = cd.dataArea ;
		var cs = getComputedStyle( da );
		var gw = parseFloat( cs.columnGap );
		var v2 = da.scrollWidth - da.clientWidth ;
		var v1 = parseFloat( cs.columnWidth ) + gw ;
		return Math.round( v2 / v1 ) + 1 ;
	}
	...
	function repaginate() {
		...
		if ( pages.length > 1 ) {
			for( var i = 1 ; i < pages.length ; i++ ) {
				cd.documentArea.removeChild( pages[ i ].dom );
			}
			pages.splice( 1 , pages.length - 1 );
		}
		
		var cs = getComputedStyle( cd.pageArea );
		var paw = parseFloat( cs.width );
		
		for( var i = 1 ; i < pageCount ; i++ ) {
			var tmp = cd.pageArea.cloneNode( true );
			tmp.removeAttribute( "id" );
			var tmp2 = tmp.querySelector( "#data-area-wrapper" );
			tmp2.removeAttribute( "id" );
			tmp2.style.left = Math.round( ( -1 * i * paw ) ) + "px" ;
			pages.push( {
				dom : tmp
			} );
			cd.documentArea.appendChild( tmp );
		}
	}
