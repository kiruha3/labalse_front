	...
	function sendXML( data , async , target , aparam ) {
	...
	function loadAgentList( url , tgt ) {
		if ( typeof tgt === "undefined" ) {
			var al = document.getElementById( "nrr-agent-sel" );
		} else {
			var al = tgt ;
		}
	...
	function agent_select() {
		...
		if ( al.selectedIndex > -1 ) {
			at.value = al.options[ al.selectedIndex ].text ;
			getAgentContacts( "ac=" + al.options[ al.selectedIndex ].value );
		} else {
	...
	function srch( tgt , tgt2 ) {
		...
		if ( at.value != "" ) {
			var omcl = [];
			var st = at.value.replace( /\s+/ , " " ).trim().split( /\s|\.|,/ );
			for( var i2 = 0 ; i2 < st.length ; i2++ ) {
				st[ i2 ] = '(\\s|\\.|\\\'|\\"|^)' + st[ i2 ];
			}
			for( var i2 = 0 ; i2 < st.length ; i2++ ) {
				st[ i2 ] = new RegExp( st[ i2 ] , "i" );
			}
			var al = document.getElementById( tgt );
			var count = al.options.length ;
			var mc = 0 ;
			for ( var i = 0 ; i < count ; i++ ) {
				var ot = al.options[ i ].text ;
				var cmc = 0 ;
				for( var i2 = 0 ; i2 < st.length ; i2++ ) {
					if ( ot.match( st[ i2 ] ) ) {
						cmc++ ;
			...
			if ( mc == 0 && st.length > 0 ) {
				at.style.backgroundColor = "#fcc" ;
			} else {
				at.style.backgroundColor = "" ;
				var j = -1 ;
				for( var i = 0 ; i < count ; i++ ) {
					if ( omcl[ i ] == mc ) {
						al.options[ i ].disabled = false ;
						if ( j == -1 ) {
							j = i ;
						}
					} else {
						al.options[ i ].disabled = true ;
					}
				}
	...
	function showEditNRRDlg( id , viewName , copy ) {
		...
		var doc = sendXML( "<get-correspondence id=\"" + id + "\" />" , false , null , "view=" + viewName );
		...
		switch( mode ) {
			case "add" :
				var xml = 
					...
					var doc = sendXML( xml , false , null , "view=" + viewName );
				break ;
	...
	function doAttacheFile( fn , id , viewName ) {
		...
		var doc = sendXML( "<get-cor-ny id=\"" + id + "\" />" , false , null , "view=" + viewName );
	...
	function showFUDlg( id , viewName ) {
		...
		var doc = sendXML( "<get-files-to-upload />" , false , null , "view=" + viewName );
	...
	function showLetterDlg( event , id ) {
		...
		const r = ldt.insertRow( -1 );
		addTabCell( r , 'ldt-cap' );
		addTabCell( r , 'ldt-cap' , 'Кому' );
		addTabCell( r , 'ldt-cap' , 'Куда' );
		
		const res = sendXML( '<get-address-list id="' + id + '" />' , false , 'correspondence.letters.php' );
		
		for( const lData of res.childNodes ) {
			let lAddresseeNode = null ;
			let lDestinationNode = null ;
			const addrType = lData.getAttribute( 'type' );
			...
			const r = ldt.insertRow( -1 );
			r.classList.add( addrType );
			const c = addTabCell( r , 'ldt-prna-btn' );
			const inp = document.createElement( 'a' );
			inp.className = '' ;
			inp.onclick = function( id , ct ) {
				return function() {
					printLetterLabel( id , ct );
				};
			}( lData.getAttribute( 'id' ) , lData.getAttribute( 'type' ) );
			const img = document.createElement( 'div' );
			img.className = 'ldt-prna-img' ;
			inp.appendChild( img );
			c.appendChild( inp );
			addTabCell( r , 'ldt-addressee' , getXMLNodeValue( lAddresseeNode ) );
			addTabCell( r , 'ldt-destination' , getXMLNodeValue( lDestinationNode ) );
		}
		...
		const cbShowHideATAddr = document.getElementById( 'letter-dlg--show-hide-at-addr' );
		cbShowHideATAddr.checked = true ;
	...
	function printLetterLabel( id , ct ) {
		...
		var lData = sendXML( '<get-letter-data docid="' + letter_dlg__selected_mat_id + '" id="' + id + '" type="' + ct + '" p1="' + pr.p1.toFixed( 2 ) + '" p2="' + pr.p2.toFixed( 2 ) + '" w="' + pr.w + '"/>' , false , 'correspondence.letters.php' );
	...
	const misa = {};
	misa[ "dr-d dr-h" ] = "dr-d dr-h hl" ;
	misa[ "dr-d dr-h hl" ] = "dr-d dr-h" ;

	function ts( event ) {
		const evt = event || window.event ;
		let tgt = evt.target || evt.srcElement ;
		if ( tgt instanceof HTMLAnchorElement && tgt.className === 'cor-files-lnk' ) {
			return ;
		}
		if ( tgt instanceof HTMLDivElement && (
			tgt.className === 'cor-b-e' ||
			tgt.className === 'cor-b-c' ||
			tgt.className === 'cor-b-a' ||
			tgt.className === 'cor-b-l b-l-at' ||
			tgt.className === 'copy-to-clipboard--anima'
		) ) {
			return ;
		}
		
		if ( tgt instanceof HTMLSpanElement && tgt.className === 'cor-reply--contact-value' ) {
			return ;
		}
		
		while ( !( tgt instanceof HTMLTableRowElement )  ) {
			tgt = tgt.parentNode ;
		}
		tgt.className = misa[ tgt.className ];
	}
	
	function copyValueToClipboard( event , val ) {
		const evt = event || window.event ;
		let tgt = evt.target || evt.srcElement ;
		
		if ( tgt instanceof HTMLDivElement && tgt.className === 'copy-to-clipboard--anima' ) {
			const parentSpan = tgt.parentNode ;
			parentSpan.removeChild( tgt );
			const newSpan = parentSpan.cloneNode( true );
			parentSpan.replaceWith( newSpan );
			tgt = newSpan ;
		}
		
		const bufferData = base64DecodeStrUTF8( val );
		navigator.clipboard.writeText( bufferData );
		
		const anima = document.createElement( 'div' );
		anima.className = 'copy-to-clipboard--anima' ;
		anima.appendChild( document.createTextNode( 'скопировано' ) );
		tgt.appendChild( anima );
	}
