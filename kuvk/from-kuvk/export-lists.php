<?php
	header( 'content-type: application/json;charset=utf-8' );

	require_once( '../../core.php' );
	/**
	 * @var array $dbConfig
	 * @var TDB $portalDB
	 */

	require_once( '../../maindb/lconfig.php'  );

	$res = array( 'success' => true , 'orgName' => iconv( 'cp1251' , 'utf8' , $dbConfig[ 'org.name.short' ] ) );
	$ccl = $portalDB->table( 'casecategory' );
	cvtArr( $ccl , 'name' );
	unsetArr( $ccl , array( 'short_name' , 'order-151-246' ) );
	$res[ 'casecategory' ] = &$ccl ;

	$toal = $portalDB->table( 'type-of-agency' );
	foreach( $toal as &$i ) {
		$i[ 'name' ] = cvt( inForm( ( !is_null( $i[ 'kuvk_label' ] ) ? $i[ 'kuvk_label' ] : $i[ 'name' ] ) , 1 ) );
		unset( $i[ '_fr' ] );
		unset( $i[ 'fmt' ] );
		unset( $i[ 'for-test' ] );
		unset( $i[ 'kuvk_label' ] );
	} unset( $i );

	$res[ 'type_of_agency' ] = &$toal ;

	$mai = $portalDB->simpleRow( 'kuvk-params' , array( 'name' => 'matincoming.add.first' ) );
	if ( $mai !== false ) {
		$mai = json_decode( $mai[ 'value' ] , true );
	} else {
		$res = array( 'success' => false );
		$res = json_encode( $res , JSON_UNESCAPED_UNICODE );
		echo $res ;
		exit ;
	}

	function getIDList2Sync( $tab , $name = false , $opt = array() ) {
		global $portalDB ;
		if ( $name === false ) {
			$name = $tab ;
		}

		if ( isset( $opt[ 'lnk' ] ) ) {
			$id = $opt[ 'lnk' ];
		} else {
			$id = 'id' ;
		}

		$l1 = $portalDB->query( 'select `t1`.`id` from `'.$tab.'` as `t1` left join `kuvk-links` as `t2` on ( ( `t1`.`'.$id.'` = `t2`.`ext_id` ) and ( `t2`.`ext_type` = "'.$name.'" ) ) where ( `t2`.`id` is null )' );
		$l2 = $portalDB->query( 'select `t1`.`ext_id` as `id` from `kuvk-sync` as `t1` where ( ( `t1`.`ext_type` = "'.$name.'" ) and ( `t1`.`processed` <> 1 ) )' );
		$res = array();
		foreach ( $l1 as &$i ) {
			$res[]= $i[ 'id' ];
		} unset( $i );
		foreach ( $l2 as &$i ) {
			$res[]= $i[ 'id' ];
		} unset( $i );
		return $res ;
	}

	function getDataWSync( $tab , $name = false , $opt = array()  ) {
		global $portalDB ;
		if ( $name === false ) {
			$name = $tab ;
		}

		if ( isset( $opt[ 'lnk' ] ) ) {
			$id = $opt[ 'lnk' ];
		} else {
			$id = 'id' ;
		}

		$did = getIDList2Sync( $tab , $name , $opt );
		if ( count( $did ) > 0 ) {
			$res = $portalDB->query(
				'select `t1`.* , `t2`.`kuvk-guid` , ( `t1`.`id` in ( ?* ) ) as `2sync` from `'.$tab.'` as `t1`
				left join `kuvk-links` as `t2` on ( ( `t1`.`'.$id.'` = `t2`.`ext_id` ) and ( `t2`.`ext_type` = "'.$name.'" ) )' ,
				'id' , '*i' , $did );
		} else {
			$res = $portalDB->query(
				'select `t1`.* , `t2`.`kuvk-guid` , 0 as `2sync` from `'.$tab.'` as `t1`
				left join `kuvk-links` as `t2` on ( ( `t1`.`'.$id.'` = `t2`.`ext_id` ) and ( `t2`.`ext_type` = "'.$name.'" ) )' ,
				'id' );
		}

		return $res ;
	}

	function cvtArr( &$arr , $col ) {
		if ( is_array( $col ) ) {

		} else {
			$col = array( $col );
		}
		foreach ( $col as $c ) {
			foreach ( $arr as &$i ) {
				$i[ $c ] = cvt( $i[ $c ] );
			} unset( $i );
		}
	}

	function unsetArr( &$arr , $col ) {
		if ( is_array( $col ) ) {
		} else {
			$col = array( $col );
		}
		foreach ( $col as $c ) {
			foreach ( $arr as &$i ) {
				unset( $i[ $c ] );
			} unset( $i );
		}
	}

	$res[ "posts" ] = getDataWSync( "posts" );
	cvtArr( $res[ "posts" ] , array( "name" , "short_name" , "simple_name" ) );

	$res[ "dep" ] = getDataWSync( "departments" );
	cvtArr( $res[ "dep" ] , array( "name" , "short_name" ) );


	$res[ "spec-gr" ] = getDataWSync( "specialities-groups" );
	unsetArr( $res[ "spec-gr" ] , "name" );
		/*$portalDB->query(
		"select `t1`.`id` , `t1`.`index` , `t2`.`kuvk-guid`
		from `specialities-groups` as `t1`
		left join `kuvk-links` as `t2` on ( ( `t1`.`id` = `t2`.`ext_id` ) and ( `t2`.`ext_type` = 'specialities-groups' ) )" ,
		"id" );*/

	$res[ "spec" ] = getDataWSync( "specialities" );
	unsetArr( $res[ "spec" ] , strexp( "{desc,actual{,_from,_to},norm{1,2,3,4},comment}" ) );
	/*$portalDB->query(
		"select `t1`.`id` , `t1`.`group` , `t1`.`num` , `t1`.`original` , `t2`.`kuvk-guid`
		from `specialities` as `t1`
		left join `kuvk-links` as `t2` on ( ( `t1`.`id` = `t2`.`ext_id` ) and ( `t2`.`ext_type` = 'specialities' ) )" ,
		"id" );*/

	$res[ "ep-states" ] = getDataWSync( "ep-states" );
	cvtArr( $res[ "ep-states" ] , array( "name" ) );
	$res[ "ep-ret-reasons" ] = getDataWSync( "ep-ret-reasons" );
	cvtArr( $res[ "ep-ret-reasons" ] , array( "name" ) );

	$res = json_encode( $res , JSON_UNESCAPED_UNICODE | JSON_FORCE_OBJECT );
	if ( $res == "" ) {
		echo json_encode( array( "success" => false , "error" => json_last_error() ) );
	} else {
		echo $res ;
	}
