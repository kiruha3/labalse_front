...
class XMLStream {
	...
	public function connect($timeout = 30, $persistent = false, $sendinit = true) {
		...
		do {
			...
			try {
				$vrcse_stream_context_options = array(
					"ssl" => array(
						"verify_peer" => false ,
				...
				$vrcse_stream_context = stream_context_create( $vrcse_stream_context_options );
				$this->socket = @stream_socket_client("$conntype://{$this->host}:{$this->port}", $errno, $errstr, $timeout, $conflag , $vrcse_stream_context );
			} catch (Exception $e) {
