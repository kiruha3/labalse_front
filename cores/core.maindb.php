<?php
	if ( !defined( 'DOCTYPE_MATINCOMING' ) ) {
		define( "DOCTYPE_MATINCOMING" , '0110' );
	}
	if ( !defined( 'DOCTYPE_PREREG' ) ) {
		define( 'DOCTYPE_PREREG'      , '0190' );
	}
	if ( !defined( 'DOCTYPE_ARCHIVE' ) ) {
		define( 'DOCTYPE_ARCHIVE' , '0010' );
	}
	if ( !defined( 'PAT_MATINCOMING_ID' ) ) {
		define( 'PAT_MATINCOMING_ID' , '/^'.VERSION_CHAR_ID.'\.\d{3}\.'.DOCTYPE_MATINCOMING.'\.20\d{8}$/' );
	}

	function getCCGroup( $id ) {
		global $TAB_CASECATEGORY ;
		return $TAB_CASECATEGORY[ $id ][ 'cc_group' ];
	}

	function isCCGZ( $id ) {
		global $TAB_CASECATEGORY ;
		if ( isset( $TAB_CASECATEGORY[ $id ] ) ) {
			return $TAB_CASECATEGORY[ $id ][ 'gz' ] == 1 ;
		} else {
			return false ;
		}
	}

	function matincomingNumberFull( $id , $depID , $caseCategoryID ) {
		$res = matincomingNumberData( $id , $depID , $caseCategoryID );
		if ( $res !== false ) {
			return implode( $res[ 'number-parts' ] );
		} else {
			return false ;
		}
	}

	function matincomingNumberFullParts( $id , $depID , $caseCategoryID ) {
		$res = matincomingNumberData( $id , $depID , $caseCategoryID );
		if ( $res !== false ) {
			return $res[ 'number-parts' ];
		} else {
			return false ;
		}
	}

	function matincomingNumberData( $id , $depID , $caseCategoryID ) {
		global $dbConfig , $TAB_CASECATEGORY , $TAB_DEPARTMENTS ;
		$fullID = ''.$id ;
		...
		if ( is_null( $depID ) ) {
			...
			$depIndex = $TAB_DEPARTMENTS[ $depID ][ 'ind' ];
		}

		$caseCategoryIndex = $TAB_CASECATEGORY[ $caseCategoryID ][ 'index' ];

		if ( isset( $dbConfig[ 'matincoming.numbering.v2.from' ] ) ) {
			$Cv2 = $dbConfig[ 'matincoming.numbering.v2.from' ];
			$n = preg_match( PAT_MATINCOMING_ID , $Cv2 );
			if ( $n == 1 ) {
				if ( intval( substr( $Cv2 , -10 ) , 10 ) <=  intval( substr( $fullID , -10 ) , 10 ) ) {
					return array(
						'number-parts' => array(
							'number' => intval( substr( $fullID , -6 ) ) ,
							'separator-1' => '/' ,
							'department' => $depIndex ,
							'separator-2' => '-' ,
							'casecategory' => $caseCategoryIndex ,
							'separator-3' => '-' ,
							'year' => intval( substr( $fullID , -8 , 2 ) )
						) ,
						'ex-data' => array(
							'version' => '2'
						)
